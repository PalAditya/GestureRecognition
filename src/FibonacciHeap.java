...
public final class FibonacciHeap<T> {
     ...
     */
    public static <T> Entry<T> mergeLists(Entry<T> one, Entry<T> two) {
     ...
     */
    public void decreaseKeyUnchecked(Entry<T> entry, double priority) {
     ...
     */
    public void cutNode(Entry<T> entry) {
    ...
    public static void main(String args[])
    {
        ...
        objts.decreaseKey(objts.enqueue(5,9),8);
        System.out.println(objts.dequeueMin().mElem);
        Entry<Integer> x=new Entry<Integer>(9,8);
        objts.enqueue(9, 8);
        objts.decreaseKey(x, 0);
        while(!objts.isEmpty())
            System.out.println(objts.dequeueMin().mElem);
        Entry dist[];
        dist=new Entry[9];
        for(int i=0;i<9;i++)
            dist[i]=new Entry<>(0,0);
    }
}